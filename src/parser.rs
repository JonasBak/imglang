...
trait TokenIterator<'a>: Iterator<Item = &'a TokenType> {
    fn peek(&mut self) -> Option<&'a TokenType>;
}
...
fn parse_primary(tokens: &mut dyn TokenIterator) -> ParserResult<()> {
    match tokens.next().ok_or(ParserError::TODO)? {
        TokenType::Number(_) => Ok(()),
